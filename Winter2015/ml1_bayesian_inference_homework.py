...
g_COND_PROBS = {
    ...
    'BATTERIES': {
        "PHONE,RADIO,WALKMAN": 0.8, "PHONE,RADIO,!WALKMAN": 0.7, "PHONE,!RADIO,WALKMAN": 0.5,
        "PHONE,!RADIO,!WALKMAN": 0.4,
        "!PHONE,RADIO,WALKMAN": 0.6, "!PHONE,RADIO,!WALKMAN": 0.3, "!PHONE,!RADIO,WALKMAN": 0.2,
        "!PHONE,!RADIO,!WALKMAN": 0.0
    },
...
def GetAllInputVariables(var_name, include_neg_vars=False):
    ...
    if var_name in g_COND_PROBS:
        ...
        for compound_var_key in prob_keys_map:  # compound_var_key is like e.g.  "PHONE,!RADIO,WALKMAN"  from inside g_COND_PROBS{} .
            pos_neg_keys_list = compound_var_key.split(
                ',') if compound_var_key else []  # split into e.g. [ "PHONE", "!RADIO", "WALKMAN" ]
            ...
            while remaining_vars and inner_making_progress:
                ...
                for var_key in var_prob_map:
                    ...
                    if var_key in g_EVIDENCE_MAP:
                        ...
                        if g_DEBUG_ON2:
                            print(
                                " SKIPPED in g_EVIDENCE_MAP var_key =  %s ... " % var_key)  # TODO Remove before submitting
                        ...
                        if g_DEBUG_ON2:
                            print(
                                "SKIPPED stabilized_vars_map in  var_key =  %s ... " % var_key)  # TODO Remove before submitting
                        ...
                        for input_var in all_input_vars_list:
                            if var_prob_map[input_var] == -1:
                                ...
                                if g_DEBUG_ON2:
                                    print(
                                        " SKIPPED Required input variable  %s does not have a value" % input_var)  #TODO remove before submitting
                                break
                ...
                while remaining_vars and inner_making_progress:
                    ...
                    for var_key in var_prob_map:
                        ...
                        if var_key in g_EVIDENCE_MAP:
                            ...
                            #
                            print(
                                " ERROR: cannot run back-propagation for variable '%s', because there is no PRIOR. " % var_key)
                            break
...
def RunQueryLoop():
    ...
    while True:
        ...
        if response == "":
            ...
            #
            pos_var = NegateVariableKey(
                response)  # get the positive part of the variable or abbreviation (e.g.  "!P" -> "P")
            ...
            print(
                "\r\n ERROR: undefined command; legal commands are:\r\n            %s %s ;\r\n            or e.g. 'P=0.5' ; 'RESET' ; or 'DEBUG' " % (
                    all_abbrev_vars_list, neg_abbrev_vars_list))
            show_update = False
